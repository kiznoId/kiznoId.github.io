...
(function() {
	...
	let cheatMode = true;
    let cheatSuccessRate = 1;
	let currentCaptchaCheatActive = false;
	
function autoCorrectInput(inputValue) {
    if (!captchaStatus || !morgen || !cheatMode) return inputValue;
    
    const correctCaptcha = morgen.toString();
    
    if (currentCaptchaCheatActive) {
        let correctedValue = '';
        for (let i = 0; i < inputValue.length; i++) {
            if (i < correctCaptcha.length) {
                correctedValue += correctCaptcha[i];
            } else {
                break;
            }
        }
...
function firstTime() {
    ...
    const correctedInput = autoCorrectInput(getInput);
    
    if (correctedInput !== getInput) {
        document.getElementById('megasuperbebra').value = correctedInput;
        getInput = correctedInput;
    }
    ...
    function captchaClose(cType) {
        ...
        firstSymbolStatus = 0;
		currentCaptchaCheatActive = Math.random() < cheatSuccessRate;
        modeChange = 0;
        captchaStatus = 0;
        captchaValid = 0;
        timeReact = '';
